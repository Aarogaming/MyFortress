...
class MyFortressIntelligenceClient:
    ...
    async def get_home_intelligence_context(self, 
                                          include_opportunities: bool = True,
                                          include_predictions: bool = True) -> HomeIntelligenceContext:
        """
        ...
        try:
            async with self.client as client:
                ...
                context = await client.get_contextual_intelligence(include_opportunities=include_opportunities)
                
                # Get home-specific recommendations
                home_recommendations = await client.get_smart_recommendations("cost")
                
                # Get optimization opportunities
                opportunities = await client.get_optimization_opportunities() if include_opportunities else []
                ...
                logger.info(f"🏠 Home intelligence context updated: health={home_context.system_health:.2f}")
                return home_context
    ...
    async def get_energy_optimization(self, 
                                    current_usage: Optional[Dict[str, float]] = None) -> EnergyOptimization:
        """
        ...
        try:
            async with self.client as client:
                ...
                energy_opportunities = [
                    ...
                    if "energy" in opp.get("title", "").lower() or 
                       "power" in opp.get("title", "").lower() or
                       opp.get("estimated_savings", 0) > 0
                ]
                
                # Calculate potential savings
                potential_savings = sum(
                    opp.estimated_savings or 0 
                    for opp in energy_opportunities
                )
                ...
                logger.info(f"🏠 Energy optimization analysis: ${potential_savings:.2f} potential savings")
                return optimization
        ...
        try:
            async with self.client as client:
                ...
                security_opportunities = [
                    self._convert_to_home_recommendation(opp)
                    for opp in opportunities
                    if "security" in opp.get("title", "").lower() or
                       "camera" in opp.get("title", "").lower() or
                       "access" in opp.get("title", "").lower()
                ]
                ...
                logger.info(f"🏠 Security intelligence updated: {len(security_opportunities)} recommendations")
                return intelligence
    ...
    async def get_predictive_automation(self) -> PredictiveAutomation:
        ...
        try:
            async with self.client as client:
                ...
                logger.info(f"🏠 Predictive automation updated: {len(automation.predicted_actions)} predictions")
                return automation
    ...
    async def get_mobile_intelligence_sync(self) -> MobileIntelligenceSync:
        ...
        try:
            ...
            logger.info(f"🏠📱 Mobile intelligence sync: {len(mobile_sync.quick_actions)} actions available")
            return mobile_sync
    ...
    async def health_check(self) -> IntelligenceHealthCheck:
        ...
        try:
            ...
            return IntelligenceHealthCheck(
                ...
                recommendation_count=len(self.intelligence_cache.get("home_context", {}).get("recommendations", []))
            )
            
        except Exception as e:
            return IntelligenceHealthCheck(
                intelligence_service_status="error",
                error_details=str(e)
            )
    ...
    def _convert_to_home_recommendation(self, opp: Dict[str, Any]) -> HomeOptimizationRecommendation:
    ...
    async def _enhance_energy_optimization(self, optimization: EnergyOptimization):
        ...
        optimization.recommended_schedule_changes = [
            {"device": "thermostat", "change": "Lower by 2°F during away hours", "savings": 12.5},
            {"device": "water_heater", "change": "Schedule heating during off-peak hours", "savings": 8.3}
        ]
    ...
    async def _enhance_security_intelligence(self, intelligence: SecurityIntelligence):
        ...
        intelligence.security_devices = [
            {"device": "front_door", "status": "secure", "last_activity": "2 hours ago"},
    ...
    async def _enhance_predictive_automation(self, automation: PredictiveAutomation, context: Dict[str, Any]):
        ...
        automation.predicted_actions = [
            {"action": "turn_on_lights", "time": "18:25", "confidence": 0.89, "reason": "typical_arrival"},
            {"action": "adjust_thermostat", "time": "22:00", "confidence": 0.76, "reason": "sleep_schedule"},
            {"action": "arm_security", "time": "08:10", "confidence": 0.92, "reason": "departure_pattern"}
        ]
    ...
    def _get_standalone_energy_optimization(self) -> EnergyOptimization:
        ...
        return EnergyOptimization(
            behavioral_recommendations=["Enable AAS intelligence integration for energy optimization"]
        )
