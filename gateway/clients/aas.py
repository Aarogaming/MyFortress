...
class MyFortressClient:
    ...
    async def call_ha_service(self, domain: str, service: str, data: Dict[str, Any]) -> Dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.post(
    ...
    async def set_ha_state(self, entity_id: str, state: Any, attributes: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.post(
                f"{self.base_url}/home-assistant/state",
                json={"entity_id": entity_id, "state": state, "attributes": attributes or {}},
                headers=self._headers()
            )
            resp.raise_for_status()
            return resp.json()
