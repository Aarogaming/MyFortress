# MyFortress

MyFortress is a subproject in the Aaroneous Automation Suite (AAS) ecosystem.
It commonly runs as a gateway/service layer and is typically included in the
full workspace as a git submodule.

## Canonical Docs

The canonical documentation lives in the AAS superproject:

- Start here: https://github.com/Aarogaming/AaroneousAutomationSuite/blob/master/docs/START_HERE.md
- Docs index: https://github.com/Aarogaming/AaroneousAutomationSuite/blob/master/docs/INDEX.md

## Repo Map

- `gateway/`: API + integrations
- `plugins/`: Plugin modules
- `tests/`: Test suite
